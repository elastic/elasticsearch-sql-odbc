...
Thanks to a standardized API, a broad range of third-party ODBC-enabled applications can now have access to {es} by using <<sql-odbc>>.
...
NOTE: The support for applications implementing the ODBC 2.x standard and prior is currently limited.
